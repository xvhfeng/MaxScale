...
void ServiceEndpoint::connect()
{
    ...
    std::unique_ptr<mxs::RouterSession> router_session;

    try
    {
        router_session.reset(m_service->router()->newSession(m_session, endpoints));
    }
    ...
    for (auto& f : filters)
    {
        ...
        if (!f.session)
        {
            throw mxb::Exception(mxb::cat("Failed to create filter session for '", f.filter->name(), "'"));
        }
...
{
    m_session->worker()->lcall([this, ref = shared_from_this(), err = std::string(errmsg)](){
